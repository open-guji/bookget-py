...
古籍数字资源下载与管理工具，支持从 45 个数字图书馆网站下载古籍图片和文字资源（包括 IIIF 2.x / 3.0 manifest）。全异步架构，插件式适配器设计。

## 支持的网站

共 **45** 个站点适配器（其中多数走 IIIF），覆盖 67 个域名。下表由适配器注册表自动生成，新增适配器后请重新生成。
...
| 東京大学東洋文化研究所 漢籍善本 (U-Tokyo) | `shanben.ioc.u-tokyo.ac.jp` |  | ✓ |  |  |

| 庆应义塾大学斯道文庫 漢籍 (Keio Shido) | `db2.sido.keio.ac.jp` | ✓ |  |  |  |
| 関西大学 KU-ORCAS (Kansai) | `iiif.ku-orcas.kansai-u.ac.jp` | ✓ |  |  |  |

| 米沢市立図書館 デジタルライブラリー (Yonezawa) | `library.yonezawa.yamagata.jp` | ✓ |  |  |  |

| 早稲田大学図書館 古典籍 (Waseda) | `wul.waseda.ac.jp` | ✓ | ✓ |  |  |
...
| 网站 | 域名 | 图片 | PDF | 文字 | 搜索 |
|------|------|:----:|:---:|:----:|:----:|
| 国际敦煌项目 (IDP) | `idp.bl.uk` `idp.bnf.fr` `idp.bbaw.de` `idp.nlc.cn` `idp.korea.ac.kr` `idp.afc.ryukoku.ac.jp` `idp.orientalstudies.ru` | ✓ |  |  |  |

### 俄罗斯

| 网站 | 域名 | 图片 | PDF | 文字 | 搜索 |
|------|------|:----:|:---:|:----:|:----:|
| 俄罗斯国立图书馆 (RSL) | `viewer.rsl.ru` | ✓ |  |  |  |

### 东南亚

| 网站 | 域名 | 图片 | PDF | 文字 | 搜索 |
|------|------|:----:|:---:|:----:|:----:|
| 喃遺産保存会 汉喃古籍 (Nom Foundation) | `lib.nomfoundation.org` | ✓ |  |  |  |
